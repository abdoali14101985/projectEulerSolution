...
public class Problem_11_LargestProductInGrid {

    public static void main(String[] args) throws IOException {
        BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(System.in));
        int gridSize = Integer.parseInt(bufferedReader.readLine().trim());
        ...
        IntStream.range(0, gridSize).forEach(i -> {
            try {
                ...
                for(int j=0;j<gridSize;j++){
                    grid[i][j] = Integer.parseInt(firstMultipleInput[j]);
                }
        ...
        printLargestProduct(grid, numberOfElements);

    }

    private static void printLargestProduct(int[][] grid, int numberOfElements) {
        ...
        for(int i=0; i< grid.length; i++){
            for(int j=0; j< grid.length; j++){
                ...
                if(j <= grid.length-numberOfElements){
                    product=1;
                    for(int k=0;k<numberOfElements;k++){
                        product *= grid[i][j+k];
                    }
                ...
                if(i <= grid.length-numberOfElements && j<= grid.length -numberOfElements){
                    product=1;
                    for(int k=0;k<numberOfElements;k++){
                ...
                if(i >= numberOfElements-1 && j<= grid.length -numberOfElements){
                    product=1;
                    for(int k=0;k<numberOfElements;k++){
                        product *= grid[i-k][j+k];
                    }
                ...
                if(i <= grid.length-numberOfElements && j>= numberOfElements-1){
                    product=1;
                    for(int k=0;k<numberOfElements;k++){
                        product *= grid[i+k][j-k];
                    }
